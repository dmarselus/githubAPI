...
import { SearchBar, Label, ListItem } from "../src/components";
...
import { ListItem as LI, Avatar } from "react-native-elements";
...
function App() {
  ...
  async function getUsername(username) {
    ...
    const url = `https://api.github.com/users/dmarselus/repos`;
    ...
    return (
      <FlatList
        ...
        renderItem={({ item: { name, owner } }) => (
          <ListItem title={name} subtitle={owner?.login} />
        )}
      />
    );
  }
  return (
    <View style={styles.app}>
      {renderHeader()}
      {renderBody()}
      <Button onPress={() => getUsername()} title="Example button" />
